...
const StakeSushi: React.FC<StakeProps> = ({}) => {
  const tokenName = "COBALT"
  const [requestedApproval, setRequestedApproval] = useState(false)

  const allowance = useAllowanceStaking()
  const {onApprove} = useApproveStaking()

  const tokenBalance = useTokenBalance(contractAddresses.cobalt[CHAIN_ID])
      ...
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            ...
            <Label text={`COBALT Tokens Available`}/>
          </StyledCardHeader>
          <StyledCardActions>
            {!allowance.toNumber() ? (
              <Button
